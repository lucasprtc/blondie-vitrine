...
const Header = () => {
    ...
    return (
        <header className="fixed top-0 left-0 w-full grid-layout grid-container py-3 z-9999">
            ...
            <div className="col-span-1 md:col-span-2 col-start-6 md:col-start-11 flex justify-end md:justify-between md:items-center">
                {menuItems.map((item, index) => (
                    <button onClick={() => handleMenuItemClick(item.sectionId)} key={index} className="hidden lg:block font-medium text-lg cursor-pointer relative group leading-6">
                        <span className="z-10 relative spacing-primary">{item.label}</span>
